...
function getChildren(parsedXML, parent)

    for a,b in pairs(parsedXML) do
        if a == "Name" and b == parent then
            table.insert(matchingTable,parsedXML)
        elseif type(b) == "table" then
            next = getChildren(b,parent)
            if next ~= nil then
                table.insert(matchingTable,next)
            end
        end
    end
